...
describe('readFiles', () => {
  ...
  it('shouldnt read two files', async () => {
    const userFiles = [
      {
        id: '1',
        name: 'index.js',
    ...
    readFile.mockReturnValueOnce(userFiles[0]);
    readFile.mockReturnValueOnce(userFiles[1]);
    await readFiles([1, 3, 8, 6])(dispatch, getState);
    expect(dispatch).toHaveBeenCalledTimes(3);
    expect(dispatch.mock.calls[0][0].type).toEqual('files/readFiles/pending');
    console.log(dispatch.mock.calls);
    expect(dispatch.mock.calls[1]).toEqual([{ type: 'files/setFiles', payload: [...userFiles, undefined, undefined] }]);
    expect(dispatch.mock.calls[2][0].type).toEqual('files/readFiles/fulfilled');
  });
});
